...
void TreeTopology::initGrids()
{
    ...
    for (TreeNode *curNode : globalTreeNodes)
    {
        ...
        yMin = min(yMin, curNodeY); // bug fixed for bucket decomposition, 2024.3.24
...
void TreeTopology::updatedGrids(int k) // k = average node count per grid
{
    ...
    for (TreeNode *curNode : globalTreeNodes)
    {
        ...
        double curNodeX = curNode->trr.getMiddlePoint().x;
        double curNodeY = curNode->trr.getMiddlePoint().y;
        xMin = std::min(xMin, curNodeX);
        xMax = std::max(xMax, curNodeX);
        yMin = std::min(yMin, curNodeY); // bug fixed for bucket decomposition, 2024.3.24
        yMax = std::max(yMax, curNodeY);
    ...
    // cout << "xunmerged: " << unmergedNodeCount << endl;
    k = std::min(unmergedNodeCount, k);
    double idealGridArea = k * (boundingBox.getArea() / double(unmergedNodeCount)); // average 4 nodes per grid

    if (unmergedNodeCount == k)
    {
        idealGridArea = boundingBox.getArea();

        gridWidth = gridHeight = max(boundingBox.getWidth(), boundingBox.getHeight());

        gridCountX = gridCountY = 1;
    }
    else
    {
        gridWidth = gridHeight = sqrt(idealGridArea);
    ...
    // int badmergenodeid;
    // if (gArg.GetInt("badmerge", &badmergenodeid))
    // {
    //     cout << "area comp: " << idealGridArea << " " << boundingBox.getArea() << endl;
    //     cout << "dimension comp: " << gridWidth << " " << gridHeight<<" "<<boundingBox.getWidth()<<" "<<boundingBox.getHeight() << endl;
    // }

    if (!(gridCountX > 0))
    {
        cout << "k: " << k << endl;
        cout << "unmerged node count: " << unmergedNodeCount << endl;
        cout << "bounding box: " << boundingBox.ll << " " << boundingBox.ur << endl;
        cout << "ideal area: " << idealGridArea << endl;
        cout << "check bb width" << boundingBox.getWidth() << " " << gridWidth << endl;
    }
    assert(gridCountX > 0);
    assert(gridCountY > 0);
    // cout << "cp3\n";
    // cout << "bbox: " << boundingBox.getWidth() << " " << boundingBox.getHeight() << endl;
    // cout << "updated grid count: " << gridCountX << " " << gridCountY <<" " <<gridCountX * gridCountY << endl;
    ...
    for (TreeNode *curNode : globalTreeNodes)
    {
        ...
        if (!globalMerged[curNode->id])
        {
            assert(boundingBox.inside(curNode->trr.getMiddlePoint()));
            addNodeToGrid(curNode->id);
        }
    }
}
        ...
        if (unmergedNodeCount > 1)
        {
            ...
            while (globalMinimumPairs.empty())
            {
                ...
                if (deadLoopCheck >= 3)
                {
                    ...
                    if (gArg.GetInt("badmerge", &badmergenodeid))
                    {
                        cout << "bad mergeeee node at: " << treeNodeGridIndex[badmergenodeid].first << " " << treeNodeGridIndex[badmergenodeid].second << endl;
                        cout << "grid now: " << gridCountX << " " << gridCountY << endl;
                    }
                    exit(0);
                }
                // cout << "remaining node count: " << unmergedNodeCount << endl;
                // cout << "grid update now!\n";
                nodeCountPerGrid *= 2;
                updatedGrids(nodeCountPerGrid);
                // cout << "grid updated!\n";
                calculateNearestNeighborForAll_BucketDecomposition(); // update the globalMinimumPairs
                if (nodeCountPerGrid > unmergedNodeCount)
                {
                    break;
                }
                // cout << "nng updated!\n";
...
void TreeTopology::calculateNearestNeighbor_BucketDecomposition(int treeNodeID) // this calculates nn for the given node using bucket decomposition(grid based method)
{
    ...
    int gridIndexY = treeNodeGridIndex[treeNodeID].second; // bug fixed for bucket decomposition, 2024.3.24
    // if (gArg.CheckExist("badmerge"))
    // {
    //     cout<<"bad merge node at: "<<gridIndexX<<" "<<gridIndexY<<endl;
    ...
    for (int i = -1; i < 2; i++) // search in 9 adjacent grids(including itself)
    {
        for (int j = -1; j < 2; j++)
        {
            if (gridExist(make_pair(gridIndexX + i, gridIndexY + j))) // bug fixed for bucket decomposition, 2024.3.24
            {
                for (int neighborID : grids[gridIndexX + i][gridIndexY + j].gridTreeNodes) // bug fixed for bucket decomposition, 2024.3.24
                {
...
bool TreeTopology::gridExist(pair<int, int> gridIndex)
{
    ...
    if (gridIndex.second < 0 || gridIndex.second >= gridCountY) // bug fixed for bucket decomposition, 2024.3.24
    {
        return false;
    }
    return true;
...
void TreeTopology::addNodeToGrid(int treeNodeID)
{
    ...
    // if (gArg.GetInt("badmerge", &badmergenodeid))
    // {
    //     cout << "node id: " << treeNodeID << endl;
    //     cout << "adding badmerge to " << gridIndexX << " " << gridIndexY << endl;
    //     // cout << "grid now: " << gridCountX << " " << gridCountY << endl;
    // }
    ...
    if (gridIndexY >= 0 && gridIndexY < gridCountY)
    {
    }
    else
    {
        cout << "checking grid boundary\n";
        cout << gridIndexY << " " << gridCountY << endl;
        cout << "curNodeY: " << curNodeY << endl;
        cout << curNodeY - boundingBox.ll.y << endl;
        cout << grids[gridCountX - 1][gridCountY - 1].ur << endl;
...
int TreeTopology::mergeNodes_BucketDecomposition(int leftChildID, int rightChildID)
{
    ...
    if (!(boundingBox.inside(mergeNode->trr.getMiddlePoint())))
    {
        // drawTRRPair("mergecheck!",mergeNode->leftChild->trr,mergeNode->rightChild->trr);
        // cout << "now noude: " << nodeCountPerGrid << endl;
        updatedGrids(nodeCountPerGrid);
        // cout << "merge node outside!\n";
        // gArg.Override("badmerge", to_string(mergeNode->id));
        // calculateNearestNeighborForAll_BucketDecomposition(); // update the globalMinimumPairs
        // cout << "merge node outsid2e!\n";
    }
    else
    {
        // cout << "good merge\n";
...
void TreeTopology::calculateNearestNeighbor() // this calculates nn for all nodes
{
    for (int i = 0; i < globalTreeNodes.size(); i++)
    {
        assert(globalTreeNodes[i]->id == i);
