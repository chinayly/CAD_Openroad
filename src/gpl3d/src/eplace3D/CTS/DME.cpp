...
{
    //! complete arboreal gradient accumulation during topDown!
    ...
    std::function<void(TreeNode *)> preOrderTraversal = [&](TreeNode *curNode)
    {
        if (curNode->leftChild != NULL && curNode->rightChild != NULL)
        {
            ...
            {
                ctsdb->clockSource = rootMergeSegment.lowerPoint;
                treeNodeLocation[curNode->id] = rootMergeSegment.lowerPoint;
                curNode->level = 0;
                // curNode->level = 1; //? start from 0 or 1?
            }
            else
            {
                TRR trr_par;
                ...
                if (merged.id == -1)
                {
                    merged = curNode->trr.core;
                    // cout<<RED<<"DEBUG RUN:\n";
                    // TRRintersectSegDebug(trr_par, curNode->trr.core);
                    // drawTRRPair("topdown", trr_par, TRR(curNode->trr.core, 0));
                    // cout << "show here Parent topdown TRR: " << trr_par << " Child TRR: " << curNode->trr << endl;
                ...
                //                        //! for details
                //                        //? switch to mid point??
                treeNodeLocation[curNode->id] = merged.getMiddlePoint();
                ...
                // curNode->accumulatedArborealGradient = curNode->parent->accumulatedArborealGradient + calculateArborealGradientFor2PinNet_WA(curNode, curNode->parent);
                ...
                //     POS_2D(treeNodeLocation[curNode->id].x, treeNodeLocation[curNode->id].y),
                //     POS_2D(treeNodeLocation[curNode->parent->id].x, treeNodeLocation[curNode->parent->id].y),
                //     POS_2D(treeNodeLocation[topology->root->id].x, treeNodeLocation[topology->root->id].y),
                //     globalDensityOverflow);
                // VECTOR_2D curLevelGradient = calculateArborealGradientFor2PinNet_WA(curNode, curNode->parent);
                POS_2D curNodeLocation(treeNodeLocation[curNode->id].x, treeNodeLocation[curNode->id].y); 
                POS_2D parentNodeLocation(treeNodeLocation[curNode->parent->id].x, treeNodeLocation[curNode->parent->id].y);
                VECTOR_2D curLevelGradient = calcWAWirelengthGradientOneNet({curNodeLocation, parentNodeLocation}, invertedGamma)[0];
                ...
                if (gArg.CheckExist("clkwirelevel"))
                {
                    ...
                    // if the current node is the longest wire in its subtree, then increase the factor
                    double curDist = L1Dist(treeNodeLocation[curNode->id], treeNodeLocation[curNode->parent->id]);
                    double leftDist = L1Dist(treeNodeLocation[curNode->parent->id], treeNodeLocation[curNode->parent->leftChild->id]);
                    double rightDist = L1Dist(treeNodeLocation[curNode->parent->id], treeNodeLocation[curNode->parent->rightChild->id]);
                ...
                curLevelGradient.x *= (levelWeight * factor);
                curLevelGradient.y *= (levelWeight * factor);
                
                // curLevelGradient.x *= (0.5 * levelWeight + 0.5 * wireWeight); 
                // curLevelGradient.y *= (0.5 * levelWeight + 0.5 * wireWeight);

                curNode->accumulatedArborealGradient = curNode->parent->accumulatedArborealGradient + curLevelGradient;
        ...
        else
        {
            ...
            curLevelGradient.x *= (levelWeight * factor);
            curLevelGradient.y *= (levelWeight * factor);
            // printf("factor: %f, curDist: %f, brotherMaxDist: %f, levelWeight: %f\n", factor, curDist, brotherMaxDist, levelWeight);
            curNode->accumulatedArborealGradient = curNode->parent->accumulatedArborealGradient + curLevelGradient;
            curNode->accumulatedPreconditioner = curNode->parent->accumulatedPreconditioner + 2 * levelWeight;
            return;
        }
...
void ZSTDMERouter::bottomUp()
{
    ...
    std::function<void(TreeNode *)> postOrderTraversal = [&](TreeNode *curNode)
    {
        ...
        { //! 的确不会有只有一个子节点的中间节点
            ...
            {
                cout << RED << "SET DELAY MODEL FIRST!" << RESET_COLOR << endl;
                exit(0);
            }
        ...
        else
        {
            ...
            assert(curNode);
            assert(curNode->id < ctsdb->dbSinks.size());
            curNode->trr.core = Segment(ctsdb->dbSinks[curNode->id]);
        }
    };
...
void ZSTDMERouter::repairSolution()
{
    ...
    if (obstacleAware)
    {
        std::function<void(SteinerPoint *)> traceToSource = [&](SteinerPoint *curNode)
        {
            ...
            for (Blockage curBlockage : ctsdb->dbBlockages)
            {
                if (curLine.direction == HORIZONTAL)
                {
                    CTSGLOBAL::CTSInterval height(curBlockage.ll.y, curBlockage.ur.y);

                    if (height.inside(curLine.ll.y))
                    {
                        CTSGLOBAL::CTSInterval width(curBlockage.ll.x, curBlockage.ur.x);
                        CTSGLOBAL::CTSInterval curLineLength(curLine.ll.x, curLine.ur.x);
...
void ZSTDMERouter::drawBottomUp()
{
    ...
    cout << BLUE << "[Router]" << RESET_COLOR << " - Visualize the bottom_up graph in \'" << outFilePath << "\'.\n";
}
...
double ZSTDMERouter::buildSolution()
{
    ...
    std::function<void(TreeNode *)> preOrderTraversal = [&](TreeNode *curNode)
    {
        ...
        if (curNode->leftChild != NULL && curNode->rightChild != NULL)
        {
            ...
            totalWirelength += L1Dist(*lcSteiner, *curSteiner);
            totalWirelength += L1Dist(*rcSteiner, *curSteiner);
    ...
    preOrderTraversal(topology->root);
    // cout << RED << "Total clock wirelength: " << int(totalWirelength) << RESET << endl;
...
void ZSTDMERouter::drawSolution()
{
    ...
    for (int tapId = 0; tapId < ctsdb->dbSinks.size(); tapId++)
    {
    ...
    cout << BLUE << "[Router]" << RESET_COLOR << " - Visualize the solution graph in \'" << outFilePath << "\'.\n";
}

void ZSTDMERouter::constructVirtualTree()
{
    topology->buildTreeUsingNearestNeighborGraph_BucketDecomposition();
    // cout << "doTopDown\n";
    totalLevelCount = countLevel(topology->root);
    topDown();
    // cout << "TopDown done\n";
}

// VECTOR_2D ZSTDMERouter::calculateArborealGradientFor2PinNet_WA(TreeNode *lowerLevelNode, TreeNode *higherLevelNode)
// {
//     Point_2D lowerLevelNodeLocation = treeNodeLocation[lowerLevelNode->id];
//     Point_2D higherLevelNodeLocation = treeNodeLocation[higherLevelNode->id];
...
//     pseudoModuleForLowerLevelNode.setCenter_2D_pseudo(lowerLevelNodeLocation.x, lowerLevelNodeLocation.y, 0.0);
//     pseudoModuleForHigherLevelNode.setCenter_2D_pseudo(higherLevelNodeLocation.x, higherLevelNodeLocation.y, 0.0);
...
int ZSTDMERouter::countLevel(TreeNode *node)
{
    if (!node)
    {
        return 0;
...
// double ZSTDMERouter::calculateLevelWeight(int level)
// {
...
//     // todo add upper and lower bound for sigma
//     sigma = min(sigma, 5.0);
//     sigma = max(sigma, 0.1);
