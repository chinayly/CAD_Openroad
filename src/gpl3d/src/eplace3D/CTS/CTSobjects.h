...
public:
    ...
    {
        os << setprecision(7) << "Seg: (" << seg.lowerPoint << "," << seg.higherPoint << ")";
        return os;
    }
...
inline double minManhattanDist(Segment ms1, Segment ms2)
{
    ...
    // if (double_equal(distance, 0.0))
    // {
    //     cout << ms1 << " " << ms2 << endl;
    // }
    assert(double_greaterorequal(distance, 0.0));
    return distance;
}
...
inline double minManhattanDist(TreeNode *nodeLeft, TreeNode *nodeRight)
{
    ...
    if (double_equal(distance, 0.0))
    {
        // cout << "overlapped node id: " << nodeLeft->id << " " << nodeRight->id << endl;
        // cout << "";
    }
    return distance;
}
...
double calculateLevelWeight(int, double, int);
double calculateWireWeight(const POS_2D& cur, const POS_2D& parent, const POS_2D& root, double globaldensityOverflow);
double calculateWireWeightFactor(const POS_2D& cur, const POS_2D& parent, const POS_2D& root, double globaldensityOverflow);
#endif