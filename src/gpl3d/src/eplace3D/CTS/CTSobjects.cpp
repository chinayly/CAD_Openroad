...
#include "CTSplot.h"
Point_2D Segment::getMiddlePoint()
{
    double x = 0.5 * (lowerPoint.x + higherPoint.x);
    double y = 0.5 * (lowerPoint.y + higherPoint.y);
    return Point_2D(x, y);
}
double Segment::slope()
{
    ...
    if (!(double_equal(slope, 1.0) || double_equal(slope, -1.0)))
    {
        cout << endl
             << slope << endl;
        cout << *this << endl;
        cout << "x comparison:" << setprecision(15) << lowerPoint.x << " " << higherPoint.x << endl;
    }
...
vector<Point_2D> TRR::getVertices()
{
    vector<Point_2D> trr_boundary_grid;
    if (double_greater(core.slope(), 0.0))
    {
...
vector<Segment> segmentCutByLine(Segment seg, double line, bool verticalOrHorizontal)
{
    ...
    if (verticalOrHorizontal)         // cut by vertical line, x=k
    {
        ...
        {
            cout << RED << "slope error(x=k)!" << RESET_COLOR << endl;
            exit(0);
        }
    }
    else // cut by horizontal line, y=k
    {
        ...
        {
            cout << RED << "slope error(y=k)!" << RESET_COLOR << endl;
            exit(0);
        }
    }
    return resultedSegments;
...
{
    CTSInterval horizontal(rectangle.ll.x, rectangle.ur.x);
    CTSInterval vertical(rectangle.ll.y, rectangle.ur.y);
...
void drawTRRPair(string name, TRR trr1, TRR trr2)
{
    ...
    cout << BLUE << "[Router]" << RESET_COLOR << " - Visualize the TRR pair in \'" << outFilePath << "\'.\n";
}

Segment TRRintersectTRR(TRR trr1, TRR trr2) //!!!!???? where was trr1 changed???? modified in ZSTDME code as well!!!
{
    // get four edges
    ...
    if (double_equal(trr1.radius, 0.0) || double_equal(trr2.radius, 0.0))
    {
        if (double_equal(trr1.radius, 0.0))
        {
            ...
            //     cout << "debughere7\n";
            //     Segment seg = intersect(trr1.core, seg2); //! seg should be a single point in most cases
            //     // ? could there be 2 intersection points for core and trr sides?
            //     cout << "returned seg id: " << seg.id << endl;
            //     cout << "returned seg: " << seg << endl;
            //     if (seg.id == 0)
            //     {
            //         return seg;
            //     }
            ...
            //         {
            //             // cout << "debughere8\n";
            //             Segment ret(seg.lowerPoint, trr1.core.lowerPoint);
            //             ret.id = 0;
            //             return ret;
            //         }
            ...
            //         {
            //             // cout << "debughere9\n";
            //             Segment ret(seg.lowerPoint, trr1.core.higherPoint);
            //             ret.id = 0;
            //             return ret;
            //         }
            //         else
...
{
    // cout << "TRR merging " << nodeLeft->id << " " << nodeRight->id << endl;
    ...
    if (!double_greater(L, 0.0))
    {
        // cout << "wrong L: " << L << endl;
    }
    assert(double_greaterorequal(L, 0.0));
    double x = solveForX(nodeLeft, nodeRight, nodeMerge, L);
    if (double_greaterorequal(x, 0.0) && double_lessorequal(x, L))
    {
        // if (nodeMerge->id == 44453)
        // {
        //     cout << "44453 went here\n";
        //     cout << "x: " << x << " L-x: " << L - x << endl;
        // }
        e_a = x;
        e_b = L - x;
    }
    else if (double_less(x, 0.0))
    {
        // if (nodeMerge->id == 44453)
        // {
        //     cout << "44453 went here2\n";
        // }
        e_a = 0.0;
        e_b = solveForLPrime(nodeLeft, nodeRight, nodeMerge, 0);
        if (!double_greater(e_b, L))
        {
            cout << RED << x << " " << e_b << " " << L << RESET_COLOR << endl;
        }
    ...
    // if (nodeLeft->id == 44452)
    // {
    //     cout << "\n44452 resulted parent MSV: " << ms_v << ""
    //          << "msv id is: " << ms_v.id << endl;
    //     cout << "44452 parent id: " << nodeMerge->id << endl;
    // }
    // if (nodeLeft->id == 44453)
    // {
    //     cout << "44453 TRR as leftchild: " << nodeLeft->trr << endl;
    // }
    // if (nodeRight->id == 44453)
    // {
    //     cout << "44453 TRR as rightchild: " << nodeRight->trr << endl;
    // }
    ...
    // if (nodeMerge->id == 44453)
    // {
    //     cout << "44453 TRR after merge: " << nodeMerge->trr << endl;
    // }
...
bool segmentOnSameLine(Segment seg1, Segment seg2) //
{
    ...
    double slope = 1.0 * (seg1.lowerPoint.y - seg2.lowerPoint.y) / (seg1.lowerPoint.x - seg2.lowerPoint.x);
...
Segment TRRintersectSegDebug(TRR trr, Segment seg)
{
    //! -1 for no intersection!
    vector<Segment> trr_Sides = trr.getBoundarys();
    for (Segment side : trr_Sides)
    ...
    vector<Point_2D> vertices = trr.getVertices();

    for (Point_2D curVertex : vertices)
    {
        cout << (seg.lowerPoint == curVertex) << endl;
        cout << (seg.higherPoint == curVertex) << endl;
    }
...
Segment intersect(Segment lhs, Segment rhs)
{
    ...
    { // equal slope
        ...
        if (segmentOnSameLine(rhs, lhs)) // check if 4 points same line
        {
            ...
            if (lower == upper)
            {
                ret.id = 1; // could be a single point intersection, which is an extreme case 2024.3.26: having trouble with this extreme case now!
            }
            else
            {
                ret.id = 0;
...
double calculateLevelWeight(int level, double globalDensityOverflow, int totalLevel) // for both DME and MMM
{
    ...
    double li = (1.0 - double(level) / double(totalLevel)) * 3; //? this is not mentioned in the paper, what should we do about this?
    // double li = 1.8 / double(level);
    double levelWeight;
...
double calculateWireWeight(const POS_2D& cur, const POS_2D& parent, const POS_2D& root, double globalDensityOverflow) // for both DME and MMM
{
    // see equation 8 and 9 in the paper "clock aware low power placement"
    // double p = 10.19;
    // double q = -2.47747;
    ...
    double curdist = abs(cur.x - parent.x) + abs(cur.y - parent.y) ;
    double rootdist = abs(cur.x - root.x) + abs(cur.y - root.y) ;
    
    double li = 3.0; 
    if ((curdist >=rootdist) || (rootdist <= 0.0)) 
    {
        li = 5.0;
    }
    assert (li > 0.0);
    li = min(( curdist / rootdist ) * 5 ,  5.0); 
    // double li = 1.8 / double(level);
    double levelWeight;
    levelWeight = (1 / (sqrt(2.0 * PI) * sigma)) * fastExp((-1.0) * ((li * li) / (2.0 * sigma * sigma)));
    return levelWeight;
...
double calculateWireWeightFactor(const POS_2D& cur, const POS_2D& parent, const POS_2D& root, double globalDensityOverflow) // for both DME and MMM
{
    // see equation 8 and 9 in the paper "clock aware low power placement"
    // double p = 10.19;
    // double q = -2.47747;
...
// double calculateDistanceWeight(int level, double globalDensityOverflow, int totalLevel) // for both DME and MMM
// {
//     // see equation 8 and 9 in the paper "clock aware low power placement"
//     // double p = 10.19;
//     // double q = -2.47747;
