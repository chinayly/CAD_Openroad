...
public: 
    ...
    double accumulateModuleDensity(function<double(const double,const double)> acc, double initial) const;
...
{
public:
    TierPlacer(PlaceDB* db, unordered_map<Module *, ModulePosition>&& position, double targetDensity = 1.0);
...
private:
    ...
    unordered_map<Module *, VECTOR_2D> clockGradientLastTime; // 上一次迭代的时钟梯度
    unordered_map<Module *, VECTOR_2D> clocknetGradient; // 时钟梯度预处理器
    unordered_map<Module *, VECTOR_2D> clockMassGradient; // 上一次迭代的时钟梯度预处理器
    ...
    void addWAClkWirelengthGradientOneNet(const Net &n, vector<WAClkWirelengthGradientCache> &cache);
    /*
        statistics
    */
    std::size_t numTiers;
    ...
    */
    ...
    void addWAWirelengthGradientOneNet(const Net& n, vector<WAWirelengthGradientCache> &cache);
...
private:
    ...
    */
    ...
    double terminalArea;
    void storeBins(int bx, int by, int bz,  double minX, double minY, double minZ, double binWidth, double binHeight, double binThickness);
    ...
    /*
        partition
    */
    void InitialPartition(std::unordered_map<Module *, ModulePosition> &moduleVector, bool isFirstCall = false);
    void preCalculateTSV(std::unordered_map<Module *, ModulePosition> &moduleVector);
    void UpdatePartitionDatabase(std::unordered_map<Module *, ModulePosition> &moduleVector);
    void SetTSVPosition(std::unordered_map<Module *, ModulePosition> &moduleVector);
    void writePlaceInfo();
    void writePartitionedPosition(std::unordered_map<Module *, ModulePosition> &partitionedPosition);
    void readPartitionedPosition(std::unordered_map<Module *, ModulePosition> &partitionedPosition);
