...
int main(int argc, char *argv[])
{
    ...
    //read part
    // 替换原有的create_partition调用
    FILE *part_fp = NULL;
    char part_filename[STR_SIZE];
    sprintf(part_filename, "%s.part", argv[1]);

    // 检查分区文件是否存在
    if ((part_fp = fopen(part_filename, "r")) != NULL) {
    fclose(part_fp);
    read_partition(part_fp, part_filename, noparts, cells, nets, cnets, &pop[0]);
    printf("Using existing partition from %s\n", part_filename);
    } else {
    // 如果没有分区文件，则创建初始分区
    float off_ratio = (float) 0.05;
    create_partition(nocells, noparts, totcellsize, max_cweight, &off_ratio, 
                        cells, nets, cnets, &pop[0]);
    printf("Created new initial partition\n");
    }
    ...
    sprintf(out_filename, "%s.part.%d", fname, noparts);
    printf("Writing partition to %s\n", out_filename);
    write_partition(out_fp, out_filename, nocells, noparts, &pop[0]);
